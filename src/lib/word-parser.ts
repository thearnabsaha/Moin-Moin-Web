...
export function parseAndCleanWords(input: string): string[] {
  ...
  for (const segment of candidateSegments) {
    ...
    const bare = hasArticle ? text.replace(/^(der|die|das)\s+/i, '') : text;
    const isAllCaps = bare.length > 1 && bare === bare.toUpperCase();
    const isTitleCase = /^[A-ZÄÖÜ][a-zäöüß]+/.test(bare);

    // If input is all uppercase e.g. "ANFANGEN", normalize to lowercase (or Titlecase if noun with article)
    let normalizedCasing = text;
    if (isAllCaps) {
      normalizedCasing = hasArticle
        ? `${text.slice(0, 4)}${bare.charAt(0).toUpperCase()}${bare.slice(1).toLowerCase()}`
        : bare.toLowerCase();
    }
    ...
    if (seenRoots.has(root)) {
      ...
      // Prefer version with article (e.g. "der Tisch" over "tisch")
      if (!existing.hasArticle && hasArticle) {
        seenRoots.set(root, { word: normalizedCasing, hasArticle: true, isCapitalized: isTitleCase });
      }
    } else {
      seenRoots.set(root, { word: normalizedCasing, hasArticle, isCapitalized: isTitleCase });
    }
  }
 ...
 * or if it was corrupted by a past naive fallback (e.g. "to (verb)", "to dank", "to heiß", "es geht" as adjective).
 */
export function isCorruptedWordData(
  ...
  // 2. Contains pseudo-meaning or placeholders
  if (m.includes('to (verb)') || m.includes('expression / phrase')) return true;

  // 3. Known naive stem strings
  if (m === 'to dank' || m === 'to heiß' || m === 'to heiss') return true;

  // 4. Naive "to <german_stem>" fallback patterns
  if (m.startsWith('to ')) {
  ...
  // 5. "es geht" misclassified as adjective or meaning "es geht"
  if (cleanW === 'es geht' && (m === 'es geht' || partOfSpeech === 'adjective')) {
    return true;
  }

  // 6. Fallback dummy sentences
  if (exampleSentence) {
    if (exampleSentence.includes('Wir müssen heute') || exampleSentence.includes('Wir verwenden den Ausdruck')) return true;
